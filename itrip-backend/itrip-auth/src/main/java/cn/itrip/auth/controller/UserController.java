...
//			user.setBaidu(userVO.getBaidu());
	...
	public @ResponseBody Dto registerByPhone(
			...
			@RequestBody ItripUserVO userVO){
		try {	
			if(!validPhone(userVO.getUserCode())){//验证手机号
				return  DtoUtil.returnFail("请使用正确的手机号注册",ErrorCode.AUTH_ILLEGAL_USERCODE);
			}
			
			ItripUser user=new ItripUser();
			user.setUserCode(userVO.getUserCode());//设置账号
			user.setUserPassword(userVO.getUserPassword());//设置密码
			user.setUserType(0);//设置用户注册类型
			user.setUserName(userVO.getUserName());//设置昵称
			if (null == userService.findByUsername(user.getUserCode())) {
	...
	private boolean validEmail(String email){
		...
		String regex="^\\s*\\w+(?:\\.{0,1}[\\w-]+)*@[a-zA-Z0-9]+(?:[-.][-zaA-Z0-9]+)*\\.[a-zA-Z]+\\s*$"  ;
