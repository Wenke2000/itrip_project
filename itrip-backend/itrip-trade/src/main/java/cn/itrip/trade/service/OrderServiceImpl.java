...
public class OrderServiceImpl implements OrderService {
	...
	public void paySuccess(String orderNo, int payType,String tradeNo) throws Exception {
		...
		itripHotelOrder.setId(itripHotelOrder.getId());
		...
		if(num>0){
			...
			int num2=itripTradeEndsMapper.insertItripTradeEnds(itripTradeEnds);
			if(num2>0){
				//通知业务模块后续处理
				//sendGet(systemConfig.getTradeEndsUrl(),"orderNo="+orderNo);

				//自己的
				sendGet(alipayConfig.getNotifyUrl(),"orderNo="+orderNo);
			}
			System.out.println("添加交易交易记录失败....");
		}

		System.out.println("添加交易编号失败....");
	}
