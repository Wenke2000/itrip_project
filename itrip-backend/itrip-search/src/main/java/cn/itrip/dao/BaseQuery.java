...
public class BaseQuery<T> {
    ...
    public Page<T> queryPage(SolrQuery query, Integer pageNo, Integer pageSize, Class clazz) throws Exception {
        ...
        Long count=docs.getNumFound();//获取结果数
        int counts=count .intValue();//转换为int类型

        Page<T> page = new Page(currPage+1, query.getRows(),counts);//查询的数据

        List<T> list = queryResponse.getBeans(clazz);//设置查询的数据
        page.setRows(list);
