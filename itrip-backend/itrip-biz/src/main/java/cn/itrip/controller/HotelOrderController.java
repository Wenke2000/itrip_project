...
public class HotelOrderController {
    ...
    public Dto<Object> addHotelOrder(@RequestBody ItripAddHotelOrderVO itripAddHotelOrderVO, HttpServletRequest request) {
        ...
        try {
            ...
            if (flag && null != itripAddHotelOrderVO) {
                ...
                itripHotelOrder.setId(itripAddHotelOrderVO.getId());//订单将来做修改
                ...
                //获取联系人[{"userName":"aa"},{"userName":"bb"},{"userName":"cc"}]
                ...
                for (int i = 0; i < size; i++) {
                    if (i != size - 1) {//判断不是最后一个，需要加，号
                        linkUserName.append(linkUserList.get(i).getLinkUserName() + ",");
                    } else {//最后一个不用加，号
                        linkUserName.append(linkUserList.get(i).getLinkUserName());
                    }
                }
                itripHotelOrder.setLinkUserName(linkUserName.toString());//设置联系人
                itripHotelOrder.setBookingDays(days);//设置预定的天数
    ...
    public Dto<RoomStoreVO> getPreOrderInfo(@RequestBody ValidateRoomStoreVO validateRoomStoreVO, HttpServletRequest request) {
        ...
        try {
            ...
            roomStoreVO.setCheckInDate(validateRoomStoreVO.getCheckInDate());//设置入住时间
            roomStoreVO.setCheckOutDate(validateRoomStoreVO.getCheckOutDate());//设置退房时间
            roomStoreVO.setHotelName(hotel.getHotelName());//设置酒店名字
            roomStoreVO.setRoomId(room.getId());//设置房间id
            roomStoreVO.setPrice(room.getRoomPrice());//设置房间的价格
            roomStoreVO.setHotelId(validateRoomStoreVO.getHotelId());//设置酒店的id

            //查询库存的业务处理
            List<StoreVO> storeVOList = tempStoreService.queryRoomStore(param);
            roomStoreVO.setCount(1);//设置默认房间数1
        ...
        if (null != currentUser) {
            ...
            try {
                ...
                if (null != hotelOrder) {
                    ...
                    //{"1":"订单提交","2":"订单支付","3":"支付成功","4":"入住","5":"订单点评","6":"订单完成"}   （systemConfig.getOrderProcessCancel()）
                    //{"1":"订单提交","2":"订单支付","3":"订单取消"} （systemConfig.getOrderProcessOK()）
                    if (orderStatus == 1) {
