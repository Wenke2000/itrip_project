...
public class ItripHotelOrderServiceImpl implements ItripHotelOrderService {
    ...
    public Map<String, String> itriptxAddItripHotelOrder(ItripHotelOrder itripHotelOrder, List<ItripUserLinkUser> linkUserList) throws Exception {
        ...
        if (null != itripHotelOrder) {
             ...
             *  二:往订单联系人表添加联系人
             */
            if (EmptyUtils.isNotEmpty(itripHotelOrder.getId())) {//判断是否已下订单
                //删除联系人（已下过订单，要修改联系人）
                itripOrderLinkUserMapper.deleteItripOrderLinkUserByOrderId(itripHotelOrder.getId());
                itripHotelOrder.setModifyDate(new Date());
                flag=itripHotelOrderMapper.updateItripHotelOrder(itripHotelOrder);//只更新了订单表

            } else {
                //没下订单
                itripHotelOrder.setCreationDate(new Date());
                flag=itripHotelOrderMapper.insertItripHotelOrder(itripHotelOrder);//只更新了订单表
                //通过订单号查询对应订单,获取订单id******************************************************************/
                String orderNo=itripHotelOrder.getOrderNo();
                Map<String,Object> orderNoMap=new HashMap<String,Object>();
                orderNoMap.put("orderNo",orderNo);
                List<ItripHotelOrder> list=itripHotelOrderMapper.getItripHotelOrderListByMap(orderNoMap);
                itripHotelOrder.setId(list.get(0).getId());
                //******************************************************************/
            }
            if (flag > 0) {
